...
from api.db.services.document_service import DocumentService, doc_upload_and_parse, doc_upload_and_parse_cover
...
def completion():
    ...
    try:
        if conv.source == "agent":
            ...
            # 增加类型检查日志
            logging.info(f"Canvas.run返回类型: {type(answer)}, 流式模式: {stream}")
            ...
            if stream:
                ...
                # goeasy 严格类型断言
                # assert isinstance(answer, partial), "流式响应必须返回partial函数，当前实际类型：{}".format(type(answer))
            ...
            #     final_ans["content"] = answer.iloc[0]["content"] if "content" in answer.columns else ""
            # else:
            #     final_ans["content"] = "\n".join(answer.get("content", []))
            # 非流式：generator 里只有一个包
            # ans = next(answer)  # 直接取第一帧
            # ✅ 收集所有 yield 的内容，取最后一个作为最终结果
            final_result = None
            for ans in answer:
                final_result = ans  # 每次覆盖，最后留下的是最终结果

            if final_result is None:
                raise Exception("Canvas 没有返回任何结果")

            final_ans.update(final_result)
            content = final_ans.get("content", "")
            # if isinstance(final_ans.get("content"), list):
            #     final_ans["content"] = "\n".join(final_ans["content"])
            # else:
            #     final_ans["content"] = str(final_ans.get("content", ""))
            if isinstance(content, pd.Series):
                content = content.dropna().astype(str).iloc[0] if not content.empty else ""
            ...
            canvas.messages.append({"role": "assistant", "content": final_ans["content"], "id": message_id})
            ref = final_ans.get("reference", [])
            if isinstance(ref, pd.Series):
                ref = ref.dropna().tolist() if not ref.empty else []
            elif not isinstance(ref, list):
                ref = []

            final_ans["reference"] = ref
            cvs.dsl = json.loads(str(canvas))
...
"""
goeasy
这里添加一个新的接口：上传文件后解析并覆盖
"""
@manager.route('/document/upload_and_parse_and_cover', methods=['POST'])  # noqa: F821
@validate_request("conversation_id")
def upload_parse_cover():
    token = request.headers.get('Authorization').split()[1]
    objs = APIToken.query(token=token)
    if not objs:
        return get_json_result(
    ...
    doc_ids = doc_upload_and_parse_cover(request.form.get("conversation_id"), file_objs, objs[0].tenant_id)
    return get_json_result(data=doc_ids)
...
"""
goeasy
这里我们修改一下接口，可以在不使用kb_id的情况下，使用token对应的dialog对应的知识库进行检索
"""
...
def retrieval():
    ...
    if req is None:
        return get_json_result(
            data=False, message='请求主体必须是有效的 JSON 数据', code=settings.RetCode.DATA_ERROR)
    ...
    try:
        if not kb_ids:
            ...
            if not objs:
                return get_json_result(
                    data=False, message='身份验证错误：API 密钥无效！"', code=settings.RetCode.AUTHENTICATION_ERROR)
            try:
                try :
                
                    kb_ids = DialogService.get_kb_ids_by_id(objs[0].dialog_id)
                    
                    if not kb_ids or kb_ids == []:
                        return get_data_error_result(message="获取知识库id为空!")
                except Exception as e:
                    return get_data_error_result(message=f"获取知识库id失败! error:{e}")
                try:                               
                    kbs = KnowledgebaseService.get_by_ids(kb_ids)
                except Exception as e:
                    return get_data_error_result(message=f"查找数据库失败:{kb_ids}")
            except Exception as e:
                return server_error_response(e)
        else:
            kbs = KnowledgebaseService.get_by_ids(kb_ids)
        ...
        if not kbs:
            return get_json_result(
                data=False, message='未找到对应的知识库', code=settings.RetCode.DATA_ERROR)
        embd_nms = list(set([kb.embd_id for kb in kbs]))
        if len(embd_nms) != 1:
            return get_json_result(
                data=False, message='知识库使用不同的嵌入模型或不存在。"',
                code=settings.RetCode.AUTHENTICATION_ERROR)
        ...
        if str(e).find("not_found") > 0:
            return get_json_result(data=False, message='没有找到块！请检查区块状态！',
...
# Compatible with skill scripts: datasets.py, update_dataset.py
# ===========================================================================

def _token_auth(func_name=""):
    """Extract and validate Bearer token from Authorization header.
    Returns (objs, error_response). If error_response is not None, return it."""
...
def _kb_to_dataset(kb) -> dict:
    ...
    return {
        ...
        "embedding_model": d.get("embd_id"),
        "chunk_method": d.get("parser_id"),
        "parser_config": d.get("parser_config"),
        "chunk_count": d.get("chunk_num", 0),
        "doc_count": d.get("doc_num", 0),
        "token_count": d.get("token_num", 0),
        "created_at": d.get("create_date", ""),
        "pagerank": d.get("pagerank"),
    }


def _doc_to_dict(doc, dataset_id=None) -> dict:
    d = doc if isinstance(doc, dict) else (doc.to_dict() if hasattr(doc, 'to_dict') else dict(doc))
    return {
        "id": d.get("id"),
        "name": d.get("name"),
        "dataset_id": dataset_id or d.get("kb_id"),
        "run": d.get("run"),
        "chunk_method": d.get("parser_id"),
        "chunk_count": d.get("chunk_num", 0),
        "token_count": d.get("token_num", 0),
...
def list_datasets():
    objs, err = _token_auth()
    if err:
        return err
    try:
        tenant_id = objs[0].tenant_id
        kbs = KnowledgebaseService.query(tenant_id=tenant_id)
        return get_json_result(data=[_kb_to_dataset(kb) for kb in kbs])
    except Exception as e:
        return server_error_response(e)
...
def create_dataset():
    ...
    if not name:
        return get_data_error_result(message="Dataset name is required.")
    try:
        tenant_id = objs[0].tenant_id
        e, _ = KnowledgebaseService.get_by_name(name, tenant_id)
        if e:
            return get_data_error_result(message=f'Dataset name "{name}" already exists.')
        ...
        if req.get("embedding_model"):
            kb["embd_id"] = req["embedding_model"]
        if req.get("chunk_method"):
            kb["parser_id"] = req["chunk_method"]
        if req.get("avatar"):
            kb["avatar"] = req["avatar"]
        if req.get("parser_config"):
            kb["parser_config"] = req["parser_config"]

        if not KnowledgebaseService.save(**kb):
            return get_data_error_result(message="Failed to create dataset.")

        kbs = KnowledgebaseService.get_by_ids([kb["id"]])
        return get_json_result(data=_kb_to_dataset(kbs[0]) if kbs else kb)
    except Exception as e:
        return server_error_response(e)
...
def delete_datasets():
    ...
    if err:
        return err
    req = request.json or {}
    dataset_ids = req.get("ids", [])
    if not dataset_ids:
        return get_data_error_result(message="ids is required.")
    try:
        tenant_id = objs[0].tenant_id
        for dataset_id in dataset_ids:
            kbs = KnowledgebaseService.get_by_ids([dataset_id])
            if not kbs or kbs[0].tenant_id != tenant_id:
                continue
            KnowledgebaseService.delete_by_id(dataset_id)
        return get_json_result(data=True)
    except Exception as e:
        return server_error_response(e)
...
def update_dataset(dataset_id):
    ...
    try:
        ...
        field_map = {
            ...
            "language": "language", "avatar": "avatar", "pagerank": "pagerank",
        }
        for req_key, db_key in field_map.items():
            if req_key in req:
                info[db_key] = req[req_key]
        if "embedding_model" in req:
            info["embd_id"] = req["embedding_model"]
        if "chunk_method" in req:
            info["parser_id"] = req["chunk_method"]
        if "parser_config" in req:
            info["parser_config"] = req["parser_config"]

        KnowledgebaseService.update_by_id(dataset_id, info)
        kbs = KnowledgebaseService.get_by_ids([dataset_id])
        return get_json_result(data=_kb_to_dataset(kbs[0]))
    except Exception as e:
        return server_error_response(e)
...
# Compatible with skill scripts: upload.py, list_documents.py, update_document.py
# ===========================================================================
...
def list_dataset_documents(dataset_id):
    ...
    try:
        ...
        docs, tol = DocumentService.get_by_kb_id(
            dataset_id, page, page_size, orderby, desc, keywords, [], [])
        return get_json_result(data={
            "docs": [_doc_to_dict(d, dataset_id) for d in docs],
            "total": tol,
        })
    except Exception as e:
        return server_error_response(e)
...
def upload_dataset_document(dataset_id):
    objs, err = _token_auth()
    if err:
        return err
    ...
    try:
        for file in request.files.getlist('file'):
            ...
            filename = duplicate_name(DocumentService.query, name=file.filename, kb_id=dataset_id)
            filetype = filename_type(filename)
            if not filetype:
            ...
            doc_result = DocumentService.insert(doc)
            FileService.add_file_from_kb(doc, kb_folder["id"], kb.tenant_id)
            results.append(_doc_to_dict(doc_result.to_json() if hasattr(doc_result, 'to_json') else doc, dataset_id))
    except Exception as e:
        return server_error_response(e)
...
def delete_dataset_documents(dataset_id):
    objs, err = _token_auth()
    if err:
        return err
    req = request.json or {}
    ...
    for doc_id in doc_ids:
        try:
            ...
            b, n = File2DocumentService.get_storage_address(doc_id=doc_id)
            if not DocumentService.remove_document(doc, tenant_id):
                errors += f"Failed to remove document {doc_id}. "
                continue
            f2d = File2DocumentService.get_by_document_id(doc_id)
            if f2d:
                FileService.filter_delete([File.source_type == FileSource.KNOWLEDGEBASE, File.id == f2d[0].file_id])
...
def update_dataset_document(dataset_id, document_id):
    objs, err = _token_auth()
    if err:
        return err
    req = request.json or {}
    try:
        ...
        if "meta_fields" in req:
            info["meta_fields"] = req["meta_fields"]
        if "enabled" in req:
            info["enabled"] = req["enabled"]

        DocumentService.update_by_id(document_id, info)
        e, updated_doc = DocumentService.get_by_id(document_id)
        return get_json_result(data=_doc_to_dict(updated_doc, dataset_id))
    except Exception as e:
        return server_error_response(e)
...
# ===========================================================================
# Parsing (Chunks)
# Compatible with skill scripts: parse.py, stop_parse_documents.py
# ===========================================================================

@manager.route('/datasets/<dataset_id>/chunks', methods=['POST'])  # noqa: F821
def start_dataset_parse(dataset_id):
    objs, err = _token_auth()
    if err:
        return err
    req = request.json or {}
    ...
    if not document_ids:
        return get_data_error_result(message="document_ids is required.")
    try:
        tenant_id = objs[0].tenant_id
        kbs = KnowledgebaseService.get_by_ids([dataset_id])
        if not kbs or kbs[0].tenant_id != tenant_id:
...
def stop_dataset_parse(dataset_id):
    objs, err = _token_auth()
    if err:
        return err
    req = request.json or {}
    ...
    try:
        ...
        for doc_id in document_ids:
            TaskService.filter_delete([Task.doc_id == doc_id])
            DocumentService.update_by_id(doc_id, {"run": 2})  # 2 = CANCEL
...
# ===========================================================================
# Retrieval Test
# Compatible with skill scripts: search.py --retrieval-test
# ===========================================================================

@manager.route('/chunk/retrieval_test', methods=['POST'])  # noqa: F821
def retrieval_test():
    objs, err = _token_auth()
    if err:
        return err
    ...
    try:
        ...
        ranks = settings.retrievaler.retrieval(
            question, embd_mdl, kbs[0].tenant_id, [kb_id], page, size,
            similarity_threshold, vector_similarity_weight, top,
            doc_ids, rerank_mdl=rerank_mdl, highlight=highlight,
            rank_feature=label_question(question, kbs))
        ...
        if str(e).find("not_found") > 0:
            return get_json_result(data=False, message='No chunk found. Please check chunk status!',
                                   code=settings.RetCode.DATA_ERROR)
        return server_error_response(e)
