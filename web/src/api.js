...
const api = {
  ...
  generateSkillContent: (params) => request('POST', '/skills/generate', params),
  ...
  deletePlan: (planId) => request('DELETE', `/plans/${planId}`),
  auto: (task, options, conv_id) => request('POST', '/auto', { task, options, conv_id }),
  ask: (task, options, conv_id, system_prompt) => request('POST', '/ask', { task, options, conv_id, system_prompt }),
